...
import { mInsight } from "../constants";
  ...
  return (
   <section className="max-container mt-[40px] md:mt-[140px]">
       ...
       <div className="flex max-lg:justify-center max-lg:items-center flex-col lg:flex-row gap-10 ">
         <h2 className="flex items-center justify-center text-center text-[40px] bg-lime-green font-grotesk py-[10px] rounded-md border-[5px] border-lime-green px-[7px] font-medium max-sm:text-[30px]">Explore More Insights</h2>
         <p className="font-grotesk flex items-center text-lg max-sm:text-center md:w-[580px] max-sm:text-[16px]">Discover expert tips, strategies, and stories to keep you ahead in the digital landscape.</p>
       </div>
   
       <div className="grid grid-cols-1 md:grid-cols-3  rounded-[45px] border-rich-black border-[1px] mt-[80px] p-[30px] border-b-[4px] ">
         {mInsight.map((study, index) => {
           ...
           >
             <h3 className="text-[24px] font-semibold mb-2 max-sm:text-[20px]">{study.label}</h3>
             <p className="text-[18px] font-grotesk mb-4 max-sm:text-[16px]">{study.desc}</p>
