...
const Awards = () => {
  return (
    ...
    <div className="flex max-lg:justify-center max-lg:items-center flex-col lg:flex-row gap-10 ">
      <h2 className="flex items-center justify-center text-center text-[40px] bg-lime-green font-grotesk py-[10px] rounded-md border-[5px] border-lime-green  max-sm:text-[30px] font-medium">Awards & Recognition</h2>
      <p className="font-grotesk flex items-center text-lg max-sm:text-center md:w-[580px]  max-sm:text-[16px]">At Positivus, our commitment to excellence has been recognized by idustry leadrs. Here are some of our proudest acheivements: </p>
    </div>

    <div className="grid grid-cols-2 lg:grid-cols-4 gap-[40px] mt-[80px]">
      {Recognition.map((recognition, index) => (
        <div key={index} className="flex flex-col items-center bg-light-gray rounded-[45px] py-[30px] px-[15px] border-rich-black border-[1px] border-b-[4px]">
          <img src={recognition.src} alt={recognition.label}  />

          <div className="flex flex-col mt-[20px]">
            <h3 className="font-grotesk font-bold text-[20px]  max-sm:text-[18px]">{recognition.label}</h3>
            <p className="font-grotesk mt-[18px] text-[15px] ">{recognition.des}</p>
          </div>
        </div>
    ...
    <div className="flex flex-row justify-between items-center flex-1 w-full bg-lime-green rounded-[45px] mt-[123px] px-[60px] mb-[23] border-[1px] border-rich-black border-b-[4px]">
        <div className="flex flex-col gap-[26px] py-[60px] max-w-[500px]">
          <h3 className="font-grotesk text-[30px]  max-sm:text-[25px]">Join Our Team</h3>
          <p className="font-grotesk text-[18px]  max-sm:text-[16px]">
