import { industriesServed } from "../constants"
  ...
  return (
    <section className="max-container mt-[40px] md:mt-[140px]">
      ...
      <div className="flex max-lg:justify-center max-lg:items-center flex-col lg:flex-row gap-10 ">
        <h2 className="flex items-center justify-center text-center text-[40px] bg-lime-green font-grotesk py-[10px] rounded-md border-[5px] border-lime-green px-[7px] font-medium  max-sm:text-[30px]">Use Cases</h2>
      ...
      <div className="w-full  mt-[80px] overflow-x-auto scrollbar-hide snap-x snap-mandatory">
        <div className="flex flex-row lg:grid lg:grid-cols-3 gap-4 sm:gap-6 min-w-[840px] lg:min-w-0">
          {industriesServed.map((study, index) => (
            <div
            key={index}
            className="flex-shrink-0 w-[280px] lg:w-full bg-black rounded-[45px] p-6 sm:p-8 snap-center"
          >
              <h2 className="font-grotesk text-sm sm:text-[25px] leading-relaxed text-white font-bold">{study.label}</h2>
               <p className="font-grotesk text-sm sm:text-base leading-relaxed text-white mt-[10px]">
                  {study.description}
                </p>
